...
class ExternalHistoryProvider(MCPProvider):
    """Example MCP provider for external game history queries."""

    name = "external_history"
    description = "查询外部历史对局库"

    def call(self, tool_name: str, params: dict[str, Any]) -> ToolResult:
        if tool_name == "query_external_games":
            return ToolResult(
                tool_name=tool_name,
                source=ToolSource.MCP_EXTERNAL,
                status=ToolStatus.SUCCESS,
...
class ExternalProfileProvider(MCPProvider):
    """Example MCP provider for external player profile queries."""

    name = "external_profiles"
    description = "查询外部玩家画像库"

    def call(self, tool_name: str, params: dict[str, Any]) -> ToolResult:
        if tool_name == "query_player_profile":
            player_id = params.get("player_id", "")
            return ToolResult(
                tool_name=tool_name,
                source=ToolSource.MCP_EXTERNAL,
                status=ToolStatus.SUCCESS,
                data={"player_id": player_id, "profile": "external_data"},
            )
        return ToolResult(
            tool_name=tool_name,
            source=ToolSource.MCP_EXTERNAL,
