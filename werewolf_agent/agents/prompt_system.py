...
# 2026-07-21 R3: 4 段纯字面常量从方法体提到 module-level 单例。
# 同进程内多次调用 _build_* 返同一字符串对象, Anthropic cache_control
# (R2) 在跨轮跨玩家复用同一 prefix byte 时, prefix-byte-matching 命中。
_STATIC_GAME_RULES: str = (
...
_NON_WEREWOLF_ROLE_RULES: dict[str, str] = {
...
def _build_role_guide_body(role: str) -> str:
    ...
    """
    if role == "werewolf":
        return WEREWOLF_ROLE_GUIDE
    if role in _NON_WEREWOLF_ROLE_RULES:
        return _NON_WEREWOLF_ROLE_RULES[role]
    return ""


@lru_cache(maxsize=8)
def _cached_role_guide(role: str) -> str:
    """R3: 7 角色分支按 role 字符串缓存, 同进程同角色玩家复用同一字符串对象.

    lru_cache(maxsize=8) 留 1 个 buffer 给未来加新角色.
    """
    return _build_role_guide_body(role)


@lru_cache(maxsize=1)
def _cached_information_boundaries(section_specs_key: tuple[tuple[str, str], ...]) -> str:
    """R3: process singleton, 同进程内只算一次.

    section_specs_key 是 _USER_SECTION_SPECS 的 (display_name, label) 元组 tuple,
    因为 _USER_SECTION_SPECS 是 module-level constant, 进程启动后恒定,
    所以这个函数实际只被算一次.
    """
    section_names = "、".join(
        name for name, _label in section_specs_key
    )
    labels = "、".join(
        dict.fromkeys(label for _name, label in section_specs_key)
    )
    return (
...
def _user_section_specs_key() -> tuple[tuple[str, str], ...]:
    """构造 _USER_SECTION_SPECS 的 hashable key, 用于 _cached_information_boundaries."""
    from werewolf_agent.agents.prompt_sections import USER_SECTION_SPECS
    return tuple((spec.display_name, spec.label) for spec in USER_SECTION_SPECS)
...
class PromptSystemMixin:
    ...
    def _build_game_rules(self) -> str:
        # R3: 纯字面常量, module-level singleton; 跨玩家跨轮返同一字符串对象.
        return _STATIC_GAME_RULES

    def _build_information_boundaries(self) -> str:
        # R3: _USER_SECTION_SPECS 是 module-level constant, _cached_* 只算一次.
        # 返回值会在所有玩家所有轮复用同一字符串对象.
        return _cached_information_boundaries(_user_section_specs_key())

    def _build_reasoning_method(self) -> str:
        return _STATIC_REASONING_METHOD
    ...
    def _build_skill_policy(self) -> str:
        ...
        """
        return _STATIC_SKILL_POLICY

    def _build_role_guide(self) -> str:
        # R3: 按 role 缓存. 同进程同角色玩家 (e.g. 4 只狼) 全部返同一字符串对象.
        return _cached_role_guide(self.context.own_role or "")

    def _build_output_contract(self) -> str:
        """Stable protocol invariants; fields come from the turn contract."""
        return _STATIC_OUTPUT_CONTRACT
