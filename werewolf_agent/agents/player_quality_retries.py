...
def _speech_quality_correction_hint(
    ...
    speech_quality 是非语义门（无幻觉脱敏顾虑），因此把上一条被拒发言原文
    回显进提示，让模型定点修改触发问题的那句、保留其余论点，而不是凭抽象
    提示整段重写——后者常再犯或改得过空，是发言反复兜底的主因之一。
    """
    ...
    elif "身份立场" in speech_quality_err or "我是好人阵营" in speech_quality_err:
        base = (
            "先补一句身份立场，例如“我是好人阵营”。"
    ...
    snippet = (rejected_speech or "").strip()
    if snippet:
        if len(snippet) > _REJECTED_SPEECH_ECHO_MAX_CHARS:
            snippet = snippet[:_REJECTED_SPEECH_ECHO_MAX_CHARS] + "…"
        base += (
            f"\n你上一条被拒发言：「{snippet}」。"
            "请只针对上述问题定点修改这句，其余论点保留，不要整段重写。"
        )
    return base
