"""Seed data loader: reads RAG entries from YAML and validates them.

The YAML file ``config/rag_seeds/seed_entries.yaml`` contains the same seed
data that was previously hard-coded in ``ingestion.py::create_seed_entries()``.
This module loads the YAML, converts each entry into an ``RAGEntry`` Pydantic
object, and runs full ``CaseIngester`` validation so the behaviour is
identical to the old inline version.
"""
