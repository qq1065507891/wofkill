...
def build_hybrid_vote_strategy(master_id: str) -> str:
    """构建混血儿投票策略。"""
    return (
        f"私下参考主人方向（master_id={master_id}），但投票理由必须伪装成普通好人视角：\n"
        "1) 观察该玩家投谁——如果对方投向有公开证据支撑，可以考虑同向投票\n"
        "2) 不要每轮都保持完全一致，否则容易暴露特殊关系\n"
        "3) 如果该玩家投的人你确实认为可疑，正常引用公开证据投票即可\n"
        "4) 如果该玩家被投，先判断其出局是否会削弱你的胜利路线，再考虑是否分票\n"
        "5) 投票理由中不要暴露特殊身份、主人关系或私有胜利条件"
    )
