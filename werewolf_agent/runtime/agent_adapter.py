...
def _estimate_witch_save_value(
    ...
    if gs.night_number == 1 and gs.day_number == 0:
        return {
            ...
            "trade_off": {
                "save_now": "首夜盲救性价比最低。被杀者约{:.0f}%概率是神职、{:.0f}%概率是普通村民——狼队首夜刀通常是随机或位置刀，被杀者是预言家的概率仅~9%。把解药用在一个村民身上浪费极大。".format(
                    power_roles_alive / max(non_wolf_alive, 1) * 100,
                    max(non_wolf_alive - power_roles_alive, 0) / max(non_wolf_alive, 1) * 100,
                ),
                "save_later": "保留解药到天亮后必有信息——警长竞选发言会暴露信息位、预言家会报查验、狼人悍跳会露出马脚。有信息后救人比首夜盲救价值高数倍",
                "risk_no_save": "首夜不救的唯一风险是被刀的就是预言家，但概率很低；且即使预言家被刀，白天也可能有悍跳狼暴露身份",
            },
        }
