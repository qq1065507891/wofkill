"""Witch poison target candidate extraction.

平衡审计 (2026-06-08) 发现女巫 4 局用毒 3 次毒好人,命中率 0%。根因是 LLM 看不到
结构化的候选目标列表,只看到模糊的"建议毒怀疑度最高的人"。

本模块提供纯函数 ``collect_witch_poison_candidates``,从公开事件中按证据
强度排序,返回 top 候选目标。

证据来源 (按权重高→低):
1. 公开查杀声明 — 真预言家/悍跳预言家在 speech 中报某玩家是狼 (最高权重)
2. 多人 (≥2) 明确指控 — 多个玩家在 speech 中说某玩家是狼
3. 警上反复自相矛盾跳位 — 同一玩家多次跳不同神职 (低权重,辅助)
"""
