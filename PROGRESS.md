...
- **本次新增 (minimax-native-key-routing)**: live API 验证发现 native MiniMax endpoint (`api.minimaxi.com/v1`) 用 `OPENAI_API_KEY`（= Ark key）会 HTTP 401。修复：`providers/openai.py` 新增 `_resolve_api_key_for_config(config, default_key)`，检测 `config.base_url` 包含 `api.minimaxi.com` 时改读 `MINIMAX_NATIVE_API_KEY` env，缺失则回退到默认 key；`PROVIDER_DOTENV_KEYS` 注册 `MINIMAX_NATIVE_API_KEY` 让 `load_local_dotenv` 加载。测试：`tests/model_gateway/test_per_profile_url_and_extra_body.py` 加 5 个 TestNativeMiniMaxApiKeyRouting 用例（monkeypatch env 覆盖 native endpoint / non-native / no-native-key fallback / no-base-url 各分支）。端到端实测：通过 `ModelRouter.from_yaml` + `OpenAIProvider` 完整代码路径调用 p01（minimax_native_m3）→ API 200，`text="OK"`, `reasoning_tokens=35`, `reasoning_status="confirmed"`。p02/p05/p07/p11 Ark 4 玩家同时实测全部 HTTP 200 走 Ark key 不受影响。
