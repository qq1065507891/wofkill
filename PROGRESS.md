...
- **本次新增 (mode-downgrade-audit-and-e2e)**: long-context 工程加固 R4 — `werewolf_agent/model_gateway/generation_attempt_context.py` 新增 `record_mode_downgrade(*, from_mode, to_mode, reason_code)` 方法 + `mode_downgrades: list[dict]` 字段 (append-only), 让 soak run 能区分"协议降级 vs 同 mode 修复". `werewolf_agent/agents/player_action_flow.py` line 463-466 之间: `prev_active_mode = active_structured_mode; new = structured_policy.next_mode(prev, reason); if new != prev: generation_attempt_context.record_mode_downgrade(prev.value, new.value, reason)`. 3 处 next_mode 调用点 (line 344 empty_response / line 381 missing_tool_call / line 471 schema_validation) 同步加审计调用. `werewolf_agent/agents/player.py` `act()` + `werewolf_agent/agents/player_action_flow.py::run_player_action_flow` 加 `generation_attempt_context` kwarg (默认 None, fresh) 让 e2e 测试观测累积; TYPE_CHECKING 规避 player ↔ player_action_flow 循环 import. module docstring 改日 2026-07-16 → 2026-07-21. 新增 `tests/agents/test_player_retry.py` 2 测试 (`test_schema_validation_triggers_next_mode_downgrade_e2e` 用 _SchemaInvalidThenValidProvider mock, 第一次返 `{"reason": 42}` 触发 Pydantic ValidationError, 第二次返合法. 配合 TestSubclass `ModelRouter` 把每个 profile 自动设 `reasoning.level=high` + `reasoning_capability=medium` 才能 router 不跳过 primary. 断言 provider.calls ≥ 1 + action.action_type == SPEECH + retry 链路不退化. `test_schema_validation_retry_includes_field_level_hint` 验证 retry 链路对 schema_validation 不崩溃且 SPEECH action 仍合法. `test_record_mode_downgrade_appends_to_context` 直接单元测 `record_mode_downgrade` 的 append + 字段语义). 共 3 测试 (含 1 个 record_mode_downgrade 单元测). 不重叠 R1 (hint 内容层) / R2 (provider payload) / R3 (prompt singleton) / 2026-07-21 三轮修复. 验证: `tests/agents/test_player_retry.py tests/model_gateway/test_generation_attempt_context.py` 6/6 PASS; 全量回归 `tests/agents tests/runtime tests/evaluation tests/cognition tests/model_gateway tests/memory tests/rag tests/integration` (不含 live + 2 latency) **4254 PASS, 2 skipped, 0 failed** (10 分 16 秒, 4251 R3 + 3 R4 新增); 1a-verify seed=100 `{p03,p06,p10,p11}` 4/4 PASS (audit 字段不影响运行时). 改动文件: `werewolf_agent/model_gateway/generation_attempt_context.py` (+`record_mode_downgrade` 方法 + `mode_downgrades` 字段 + docstring 改日)、`werewolf_agent/agents/player_action_flow.py` (+`generation_attempt_context` kwarg + 3 处 `record_mode_downgrade` 条件审计调用)、`werewolf_agent/agents/player.py` (`act()` 加 kwarg forward + TYPE_CHECKING import)、`tests/agents/test_player_retry.py` (+187 行 / 2 e2e 测试 + 测试基类 _SchemaInvalidThenValidProvider 与 _schema_invalid_routing). 开放风险/待办: (a) router 在每次 attempt re-resolve mode, provider 实际收到的 mode 不一定与 R4 记录的 policy.active_structured_mode 一致; 这是 router 既有设计, R4 不修正 (audit 仅记录 policy 层 transition); (b) `provider.calls` 显示 router 实际 mode 没变 (连续 3 次都是 json_schema), 说明 `allow_text_tool_fallback=True` + provider=r4prov 让 router 进入 TEXT_JSON 默认而非 JSON_OBJECT 降级, 印证 (a); (c) `mode_downgrades` schema 暂未冻结 enum (mode 和 reason_code 都是 plain string), 后期需固化以匹配 provider 协议枚举; (d) R5 (strategy_directive LRU budget, 58 key 按 HARD/SUGGEST/REFERENCE 三档预算裁剪) 是后续候选.
- **本次新增 (system-prompt-static-section-singleton)**: long-context 工程加固 R3 — `werewolf_agent/agents/prompt_system.py` 把 6 段稳定 system prompt builder 抽为 module-level 单例, 让 R2 的 Anthropic cache_control 在跨玩家跨轮复用同一字符串对象: 4 段纯字面常量 (`game_rules` / `reasoning_method` / `skill_policy` / `output_contract`) 提成 module-level `_STATIC_GAME_RULES` 等字符串常量, 原方法体退化为 `return _STATIC_FOO`; `_build_information_boundaries` 用 `@lru_cache(maxsize=1)` 按 `tuple((display_name, label))` 缓存, `_USER_SECTION_SPECS` 是 module-level constant, 进程启动后只算一次; `_build_role_guide` 用 `@lru_cache(maxsize=8)` 按 `role` 字符串缓存, 同进程同角色玩家 (4 只狼 / 3 村民 / ...) 全部返同一字符串对象; `_NON_WEREWOLF_ROLE_RULES` 字典升 module-level, `WEREWOLF_ROLE_GUIDE` 已在 `wolf_prompt_contract.py:25-33` 是 module-level constant. module docstring 修改日期 2026-07-18 → 2026-07-21. 不重叠 R1 (hint 内容层) / R2 (provider payload + UsageRecord cache_*) / 2026-07-21 三轮修复 (planning envelope / target_stance_contract / jitter retry). 新增 `tests/agents/test_prompt_builder.py` 5 测试 (`test_static_literal_sections_return_singleton_string_objects` 4 段 cross-instance `is` 同一对象 + 长度 > 100 / `test_information_boundaries_process_singleton_across_calls` 不同玩家两次调返同一对象 / `test_role_guide_per_role_singleton` 同角色同对象不同角色不同 / `test_static_sections_keep_existing_string_content` 内容跟 R3 前等价不变 / `test_static_section_overlap_with_r2_cache_marker_does_not_change` R2 cache marker 仍在). 修复前 1 FAIL (`test_information_boundaries_process_singleton_across_calls` 因为 f-string 拼接每次产生新字符串), 修复后 5/5 PASS. 验证: prompt 相关 208 测试零回归; `tests/agents tests/runtime tests/evaluation tests/cognition tests/model_gateway tests/memory tests/rag tests/integration` (不含 live + 2 latency) **4251 PASS, 2 skipped, 0 failed** (10 分 00 秒, 零回归, 4251 = 4246 R2 末态 + 5 R3 新增); 1a-verify seed=100 `{p03,p06,p10,p11}` 4/4 PASS (LLM 行为零变化). 改动文件: `werewolf_agent/agents/prompt_system.py` (重写为 singleton + lru_cache, docstring 改日), `tests/agents/test_prompt_builder.py` (+102 测试). 协同收益: R2 的 cache_control 现在跨玩家/跨轮命中同一 prefix 对象, Anthropic prefix-byte-matching 命中度更高, `cache_read_input_tokens` 占比增大. 开放风险/待办: (a) `_lru_cache(maxsize=1)` 在 `_USER_SECTION_SPECS` 被未来测试 monkeypatch mutate 时仍返回旧值 (lru_cache 自身缓存), 需要 lru_cache.clear() 或 hot reload; 实际中 `_USER_SECTION_SPECS` 是 module-level constant, 不期望 mutate; (b) `information_boundaries` 段 cross-process 不复用 (lru_cache 是 per-process), 跨进程 cache 仍靠 Anthropic server-side 5min TTL; (c) 6 段内容提升后, 真实 cache_hit 比例需在 soak 中观测 — Anthropic response.usage.cache_read_input_tokens 字段已经 R2 接好, 等下次 soak run 验证; (d) `_cached_role_guide` cache key 是 str, 没有 owner 概念, 进程内同 role 返回同一对象, 不同 role 返回不同. 下一轮候选: R4 (StructuredOutputPolicy.next_mode 路径 e2e 测试) / R5 (strategy_directive LRU budget, 58 key 按 HARD/SUGGEST/REFERENCE 三档预算裁剪).
- **本次新增 (anthropic-prompt-cache-control)**: long-context 工程加固 R2 — `werewolf_agent/model_gateway/providers/anthropic.py` + `providers/minimax.py` 把 `system_prompt` 从裸 str 升级为 Anthropic 2026 协议的 list-of-text-blocks 形式 `[{"type":"text","text":...,"cache_control":{"type":"ephemeral"}}]`，跨轮跨玩家复用 `cache_read_input_tokens`（写 1.25×/token, 读 0.1×/token, TTL 5 分钟）。新增 helper `_wrap_system_prompt_for_cache(system_prompt)` (anthropic.py), minimax 直接 `from ...anthropic import _wrap_system_prompt_for_cache` 复用，结构 100% 对称。新增 `_system_bytes_for_observer(payload.get("system"))` 处理 list/str 双形态（JSON dumps 给 list, encode 给 str），保持 `validate_final_prompt_contract` 字节匹配仍然命中。`UsageRecord` 新增 `cache_creation_input_tokens` + `cache_read_input_tokens` 字段（默认 0，frozen dataclass），anthropic / minimax 解析 response.usage 后写入；`_BaseHttpProvider._usage` 加 2 个 kwarg。改动文件: `werewolf_agent/model_gateway/providers/anthropic.py`(+_wrap_system_prompt_for_cache / _system_bytes_for_observer / docstring 改日 2026-07-15→2026-07-21)、`providers/minimax.py`(+同步调用 + docstring 改日)、`providers/base.py`(_usage 加 2 kwarg)、`usage_records.py`(UsageRecord 加 2 字段, docstring 改日 2026-07-21)、`tests/model_gateway/test_anthropic_provider.py`(+TestAnthropicPromptCache 5 测试: 长 prompt 走 list + cache_control / 短 prompt 也走 list / None 不发射 / usage parse 含 cache 字段 / 老 vendor 缺字段 0 default)、`tests/model_gateway/test_minimax_provider_routing.py`(+2 测试: minimax payload 同步 / 短 prompt 也走 list)、`tests/model_gateway/test_final_prompt_observer.py`(2 现有测试更新 system 字节断言兼容 list 形态)、`tests/agents/test_model_router.py`(`test_anthropic_provider_posts_messages_request` 更新 system 期望为 list 形态)。验证: model_gateway + R1 tests 399 PASS；`tests/agents tests/runtime tests/evaluation tests/cognition tests/model_gateway tests/memory tests/rag tests/integration`(不含 live + 2 latency)**4246 PASS, 2 skipped, 0 failed**(10 分 03 秒, 零回归)；1a-verify seed=100 `{p03,p06,p10,p11}` 4/4 PASS (cache marker 不动 prompt 渲染)。不重叠 R1 (hint 内容) / C 节三轮修复 (target_stance contract / jitter retry)。开放风险/待办: (a) Anthropic ephemeral cache 最小 1024 token 起步, 短 prefix 加 marker 仍合法但 Anthropic 静默忽略, 不会创建 cache; R2 选择统一形态 (所有 system_prompt 都走 list) 是简化实现, 优化交给 R3 (system 静态段抽 per-player 不变单例扩 prefix); (b) R2 当前在 anthropic provider 层加 marker, Anthropic 同时允许在 messages 末尾 + tools 字段上加 marker. R2 只放 system, 跨轮 message 缓存留后续轮次; (c) 目前 UsageRecord cache 字段写完后无 dashboard 渲染, 未来需要在 monitoring-closure 链路接出 cache_hit_rate 指标。
- **本次新增 (schema-validation-field-level-retry-hint)**: long-context 工程加固 R1 — `_parse_correction_hint` 当前对 Pydantic schema-validation 错误 (`parse_error.startswith("Schema validation error:")`) 回吐空泛语"只输出 JSON..."，LLM 第二次容易撞同样的错。新增 `build_schema_validation_hint(parse_error, max_items=5, max_chars=800)`：按 Pydantic v2 `ValidationError.__str__()` 标准格式 (实测 2.13 `1 validation errors for ...`, 第二行 `Field required [type=missing, input_value=None, input_type=NoneType]`, 第三行 url) 用两条 regex (`_PYDANTIC_VALIDATION_HEADER` / `_PYDANTIC_BLOCK_*_LINE`) 解析出 (loc, msg, type, input) 列表，输出"违规字段 (最多 5 条): - 路径 `target_stance`: Field required (当前值: None)"的字段级 hint；非 Pydantic 类型或解析失败返回空串让 caller fallback 到原兜底语。`player.py::_parse_correction_hint` 加分支前置：先试 `build_schema_validation_hint`，有具体字段信息就用，否则走 sheriff_vote 分支。`player_retry_hints.py` 模块 docstring 修改日期 2026-07-08 → 2026-07-21。新增 `tests/agents/test_player_retry.py` 3 测试：`test_build_schema_validation_hint_extracts_field_paths`（验 target_stance / reason 字段路径 + 具体 msg 在 hint 里）/ `test_build_schema_validation_hint_truncates_long_errors`（60 个字段违规只展示前 5，末尾 "另有 55 条违规未列出"）/ `test_build_schema_validation_hint_returns_empty_on_non_pydantic_input`（`truncated_json: ...`、空串、纯文本都返回空）。修复前 3 测试全 FAIL（ImportError），修复后全 PASS。验证：单测试套件 `tests/agents/test_player_retry.py + test_player_agent.py` 158 PASS（含 3 新增）；全量回归 `tests/agents tests/runtime tests/evaluation tests/cognition tests/model_gateway tests/memory tests/rag tests/integration`（不含 live + 2 latency）**4239 PASS, 2 skipped, 0 failed**（10 分 15 秒，零回归）；1a-verify seed=100 `{p03,p06,p10,p11}` 一次跑 4/4 PASS（hint 不进 WOLF_DISCUSSION 路径，故不影响 target_stance 协议层）。改动文件：`werewolf_agent/agents/player_retry_hints.py`(+172 / module docstring 改日 2026-07-08→2026-07-21)、`werewolf_agent/agents/player.py`(+1 import + 8 行 `_parse_correction_hint` 提前分支 + 调用 build_schema_validation_hint)、`tests/agents/test_player_retry.py`(+83 测试)。不重叠 2026-07-21 三轮修复 (planning 透传 / target_stance_contract 必填指令 / agent_wolf_actions.py jitter retry)：R1 改 hint 内容层、caller 仍走 player_action_flow.py:463 的 `structured_policy.next_mode(...)` 现有降级；不动 envelope、不动 directives、不动 jitter。开放风险/待办：(a) Pydantic v3 若改 `__str__` 格式需更新 regex；(b) hint 800 char 上限对小众 schema 一次 10+ 字段违规可能裁过头，需 watch audit；(c) 当前 hint 在 player.py:154-167 处于"已注入但未走测试"——尚未写 e2e 测试覆盖"input 是真 ValidationError 时整轮 .act() 重发后第二次能拿到 hint"路径，需要 R4 或单独一轮补 integration 测试。下一轮 R2：Anthropic provider 加 `cache_control: ephemeral` 标记（依赖 R1 不变，独立可 merge）。
- **本次新增 (wolf-discussion-target-stance-jitter-retry)**: prompt HARD_KEYS 修复后 1a-verify 仍偶尔 jitter（实测 seed=100 连续 5 跑有 1 次 p06/p10 静默漏 target_stance；剩余 4 次 LLM 在 1 次重试后必填）。在 `agent_wolf_actions.py` 的 `agent_wolf_discussion` 加单次 jitter 兜底：(1) 模块常量 `_STANCE_RETRY_HINT`（精简版 contract，强调"上次漏字段、本次必须填、abstain 也算合法"）；(2) 第一次 `agent.act(context)` 后若 `target_stance is None`，用 `context.model_copy(update={"strategy_directive": {..., "target_stance_retry_hint": hint}})` 重新调一次 `agent.act(retry_context)`；(3) retry 期间 `exposure_collector._append("wolf_target_stance_retry", identity, {wolf_id, round, retry_filled_stance})` 用于 soak 抖动率观测。新增 `tests/runtime/test_wolf_prompt_contract.py` 3 测试：retry 才发生 / 已有 stance 不重试 / 写审计事件（修复前 2 FAIL，修复后 PASS）。`scripts/verify_wolf_discussion.py` 加 retry 计数读出 (decisions_identity+exposure_collector) 让 1a-verify 输出能区分"零次通过 / 一次重试通过 / 仍然漏"，决策日期 2026-07-21。1a-verify seed=100 {p03,p06,p10,p11} 连跑 5 次：**每次 4/4 PASS，0 failures**；分布上每个 wolf 约 20-40% 概率需要 1 次 retry，retry_filled_stance=True 100%；从不出现两次 retry 都不出 stance 的情形（说明单次 retry 已足够）。改动文件：`werewolf_agent/runtime/agent_wolf_actions.py`(+_STANCE_RETRY_HINT+retry块+模块 docstring 改日 2026-07-18→2026-07-21)、`tests/runtime/test_wolf_prompt_contract.py`(+181 行 / 3 测试)、`scripts/verify_wolf_discussion.py`(+decision_identity 注入+retry_count 读出)。回归：`tests/agents tests/runtime tests/evaluation tests/cognition tests/model_gateway tests/memory tests/rag tests/integration`（去掉 live + 2 个 latency）**4236 PASS, 2 skipped, 0 failed**。开放风险/待办：(a) 单次 retry 假设 attempt-1 失败但 attempt-2 必出——理论上极端模型在 retry 期间又漏字段；目前 run_player_action_flow 内的 max_retries=3 仍未触发，因为 silent missing 不是 schema validation error，无 retry signal；如果未来 soak 出现连续 2+ 次 jitter，需在 protocol 层加 hard rejection + repeat-until-filled (open)；(b) `_STANCE_RETRY_HINT` 是中文硬编码；如果未来切换英文 prompt 需同步；(c) retry 路径在 audit collector 上新写 `wolf_target_stance_retry` 事件，尚未走 GameEvent 持久化路径——只对 verify 暴露可见，不进入 game replay log；(d) 实测 p06/p10 在 retry 后经常产出 `stance=abstain` 而非 `propose`，原因是模型在加压下保守——这是可接受行为，等下一轮 round 2/3 才提议具体目标。
- **本次新增 (wolf-discussion-target-stance-prompt-harden)**: planning 层透传修复（上一条）1a-verify 暴露"必要但不充分"——MiniMax-M3 native / Kimi-K2.6 在 plan envelope 模式下从不主动产出 `target_stance` 字段，schema required 形同虚设。两阶段加固：(1) `werewolf_agent/runtime/wolf_discussion_directives.py` 的 `build_wolf_discussion_strategy_directive` 新增 `target_stance_contract` 键（必填 + 合法枚举 + 严禁省略 + abstain 时 target_id=null），与 `wolf_team_discussion` 一起喂给 LLM；(2) **关键** — `werewolf_agent/agents/directive_priority.py` 把 `target_stance_contract` 加入 `HARD_CONSTRAINT_KEYS`，否则会被 `PromptStrategyMixin` 渲染到最低优先级【参考】块，LLM 当背景字段忽略。第一轮我把 contract 落地但忘了分类，结果 verify 4 狼中 p06/p10 仍然空 stance；提升 HARD 后重跑 verify 稳定 4/4 PASS（含 p06/p10 选 abstain 也是合法 stance 输出，差别从"字段缺失"变成"主动 abstain"，远比空字段好）。新增 `tests/runtime/test_wolf_discussion_directives.py` 2 测试（基线 contract 必填+合法枚举 / layered_context 注入路径保留 contract）。改动文件：`werewolf_agent/runtime/wolf_discussion_directives.py`(+26/-2, module docstring 改日 2026-07-18→2026-07-21)、`werewolf_agent/agents/directive_priority.py`(+9/-1, 改日 2026-07-05→2026-07-21)、`scripts/verify_wolf_discussion.py`(+`--seed` 参数, 改日 2026-07-20→2026-07-21)、`tests/runtime/test_wolf_discussion_directives.py`(+48 测试)。验证：本次 2/2 PASS；`tests/agents/test_prompt_strategy.py + test_prompt_builder.py + test_prompt_mode_isolation.py + tests/runtime/{test_wolf_discussion_directives,test_wolf_flow,test_strategy_directives,test_wolf_prompt_contract} + tests/agents/test_planning.py` 389 PASS；`tests/runtime/` wolf 相关 345 PASS；`tests/integration/test_*wolf*` 24 PASS。1a-verify 多 seed 多 wolf 实测：seed=42 round=1 {p01,p04} 2/2 PASS；seed=42 round=2 {p01,p04} 2/2 PASS；seed=1 round=1 {p01,p04,p08,p11} 4/4 PASS；seed=100 round=1 {p03,p06,p10,p11} 4/4 PASS（含两个 LLM 选 abstain）。开放风险/待办：(a) 模型不稳——同一 seed 跑两次出现过一次 p06/p10 stance=None（不稳定 jitter），但只是从"该狼本轮 abstain"变成"LLM 当次忘了字段"；对此走协议层 retry 仍是空白；(b) `target_stance` LLM 仅在 schema 提示下产出，**LLM 仍可能产错优先级（如 backup 选成 primary）**或**不合法字符（如多分隔的 id 列表）**，当前不做归一化，靠 `build_validated_wolf_target_stance` 在 `derive_wolf_consensus_evidence` 聚合时自动忽略非法 stance；(c) soak 实战：完整 game replay（`g_xxxxxxx` 多夜）跑下来是否仍频繁空刀只能靠之后 run-soak 验证。
- **本次新增 (wolf-discussion-target-stance-passthrough)**: 狼人讨论产生空刀（`_planned_wolf_kill primary: status=all_abstain target=None supporters={}`）根因：planning envelope 路径下 `decision_and_dialogue_to_action` 直接构造基类 `PlayerAction`（WOLF_DISCUSSION + SPEECH），丢掉 LLM 的私有 `target_stance` 字段 → `agent_wolf_actions.py:586` `getattr(action, "target_stance", None)` 永远 None → `build_validated_wolf_target_stance` fallback 到 `{target_id: None, stance: "abstain", priority: "primary"}` → `derive_wolf_consensus_evidence` 聚合时 `stance in {"propose","support"} and target_id is not None` 全过滤 → `all_abstain` → `strategic_abstain` 空刀；N1 修复 ff054a1 只解决了 text/stance 协议层脱钩（提前结束），未改 planning 层。方案 A（用户选定）：`agents/planning.py` 的 `planning_envelope_to_action` 从 envelope `data.get("target_stance")` 抽出 `WolfTargetStanceAction` 透传，并 audit 记录；`decision_and_dialogue_to_action` 新增可选 `target_stance` kwarg + WOLF_DISCUSSION+SPEECH 分支构造 `WolfDiscussionSpeechPlayerAction`（基类 SpeechPlayerAction 没这个字段，必须落到 wolf 子类），传递 target_stance=None 时仍是合法 wolf action（不带 stance 字段）。新增 `tests/agents/test_planning.py` 3 测试 (envelope 有 stance→透传 / envelope 无 stance→wolf 子类+stance=None / 内层直接调用 target_stance 形参)；修复前 3 个全 FAIL，修复后 PASS。改动文件：`werewolf_agent/agents/planning.py`(+13 / -2, module docstring 修改日期 2026-07-05→2026-07-21)、`tests/agents/test_planning.py`(+91, +2 imports)。验证：本次 9/9 PASS；`tests/agents/*` 230 PASS；`tests/runtime/*` wolf/vote/night/strategy 531 PASS, 1 skipped；`tests/integration/test_*wolf*` 24 PASS；全量 5266 PASS, 2 skipped, 0 failed（13 分 09 秒，零回归）。开放风险/待办：1a-verify 真实对局下 target_stance 出结构化字段的概率/正确性仍是模型战术层议题（推理模型在 plan 信封模式下需 prompt 引导写入 target_stance 字段，schema 已声明 required，模型实际输出待 soak 验证）；验证脚本: `python scripts/probe_player_models.py` 应能验证 wolf-discussion envelope 模型是否产出 target_stance。
- **本次新增 (strip-think-tags-minimax-native)**: `_generate_openai_compatible` 出口新增 `_strip_thinking_prefix` 剥离 MiniMax native 端点的 `<think>` XML 嵌入, 标准化回 OpenAI Chat Completions 的 `content`/`reasoning_content` 字段分离契约。`GenerateResult` 新增 `thinking_text: str = ""` 字段统一承载 Ark `reasoning_content` 和 MiniMax `<think>` 剥离内容的推理链原文供审计。改动文件: `werewolf_agent/model_gateway/providers/openai.py` (+`_strip_thinking_prefix` 函数 + `_generate_openai_compatible` 调它, +删 `_openai_chat_completions_url` 局部 `import re`)、`werewolf_agent/model_gateway/usage_records.py` (+`thinking_text` 字段)、`tests/model_gateway/test_providers.py` (+9 测试: 7 `_strip_thinking_prefix` + 2 `GenerateResult` 默认)。验证: `tests/model_gateway/ tests/agents/ tests/runtime/ tests/integration/` **3022 passed, 2 skipped** (`-o addopts=""` 串行, 零回归)。3 commits: `d01f6d5` feat + `3ef77fc` chore(docstring dates) + `0285354` test。开放风险: 1a-verify 真实 LLM 验证需 API key 环境, 用户本地有 key 时跑 `python scripts/probe_player_models.py` 应全部 PASS (MiniMax native text 干净, thinking_text 含推理链)。修复 5/12 玩家 (p01/p03/p06/p08/p10 MiniMax native) 的 text 污染。
- **本次新增 (speech-quality-retry-echo)**: 用户显式扩大 scope（非 Task 15）——诊断累积 game_stdout.log（07-12~07-20，13 局）185 次 exhausted→fallback，其中 SPEECH 166；最终失败 `last_error` 分布 speech_quality 121 (65%) / semantic_claim_retention 56 (30%) / schema 5 / illegal 3；单局 g_714613031 发言兜底率 21/57 ≈ 37%。根因：重试只回传抽象 `correction_hint`（不含被拒发言原文），模型近似原样重发 → `check_repeat_error_signature` 两次签名相同 → 直接 break 兜底（常连 3 次都跑不满）。修复两点：① `player_quality_retries._speech_quality_correction_hint`/`build_speech_quality_retry` 新增 `rejected_speech` 参数（默认 ""，向后兼容），把被拒发言原文（上限 120 字截断）拼进 hint 并要求“定点修改这句、其余保留、不要整段重写”；`player_action_flow.py:558` speech_quality 分支传入 `rejected_speech=action.speech`。② 同分支移除 `repeat_error_signature` 短路 break（改为重置 `last_error_signature=None` 后 `continue`），speech_quality 靠 `max_retries` 自然兜底，不再因两次措辞近似就放弃修复。speech_quality 是非语义门、无幻觉脱敏顾虑，故回显原文安全（语义门 `unsupported_public_claim` 脱敏行为不动）。改动文件：`werewolf_agent/agents/player_quality_retries.py`、`werewolf_agent/agents/player_action_flow.py`、`tests/agents/test_player_agent.py`（+2 测试：回显原文 / 无原文向后兼容）。验证：`tests/agents/test_player_agent.py -k "speech_quality or rejected_speech"` 8 passed；`test_player_agent + test_player_retry + test_prompt_injection_fixes + runtime/test_speech_quality + regression/test_post_july14_contract_cases` 219 passed；`tests/agents` 全量 961 passed（零回归，`-o addopts=""` 串行）。开放风险/待办：(a) 仅单元+集成验证，未跑真实对局实证兜底率是否实际下降（1a-verify）；(b) 观测盲区未修——`semantic repair rejected` warning 只在 `semantic_repair_source is not None` 分支打（56 次语义门最终失败仅 1 条 warning），本次未动，建议后续把 log 移到 `error_code=="semantic_claim_retention"` 赋值处；(c) speech_quality 现固定跑满 max_retries，最坏多 1-2 次 LLM 调用/次失败，属预期成本。
- **本次新增 (stale-branch-cleanup)**: `git branch --no-merged master` 报告 3 个未合并分支（`codex/srp-rag-splitting` 2 commit / `codex/srp-file-splitting` 21 commit / `codex/system-audit-repair-20260710` 17 commit）。独占价值盘点：srp-rag-splitting 的 21 个 `*_extended.py` 测试不在现行代码规范 + 8 张开发截图；srp-file-splitting 的 10 个生产代码（game_cognition/game_config 路由、metric_sections、good_last_words_handler）与 master 后续演进（post-july14 + semantic-public-evidence-gate）已大量重叠；system-audit 的 4 个新 runtime 模块（offline_reflection/state_commit/terminal_state/public_reason）+ 6 个设计文档 + 6 个测试全部为 branch 独占（master 上不存在），但与 post-july14 修复（`4b5e700/c7b9ae2/70ce411` 等 resolution_batch / decision attempt 修复）方向不同。3 个分支切出后 master 独立演进 ~50 commit，逐分支 merge-tree 冲突文件 20+，强行合并成本 2-3 小时且引入回归风险。按用户决策，3 个分支全部删除 (`git branch -D ...`)，独占代码/文档留作后续独立 cherry-pick 候选（system-audit 的 design/spec/plan 文档保留在分支 SHA `f405df5` 起点），无 worktree 绑定所以无需清理 worktree。Post-cleanup `git branch --no-merged master` 空输出，master HEAD `584cc68` 保持不变。
- **本次新增 (semantic-public-evidence-gate-merge)**: `codex/semantic-public-evidence-gate`（3 commit：`312b8e7` enforce night info polarity / `f60ef03` support first-person night info evidence / `c6256a0` scan all night info evidence matches）经 fast-forward 合并回 `master`，HEAD = `c6256a0`。改动落在 `werewolf_agent/evaluation/balance_public_claims.py`（night info evidence 扫描/极性/首人称 support）+ 两个测试文件（`tests/agents/test_player_retry.py` +50、`tests/evaluation/test_game_balance_batch.py` +46）。Post-merge 核心回归：在主仓库根目录（隔离 worktree 已切回前清理）`python -m pytest -o addopts="" -q` → **5256 passed, 2 skipped, 1 failed**（单 failure 为 `tests/test_repository_hygiene.py::test_runtime_graph_has_no_duplicate_top_level_function_names`，根因 `werewolf_agent/runtime/graph.py` 文件首部 UTF-8 BOM（`efbbbf`）触发 `ast.parse` `SyntaxError`，与本次合并无关，pre-existing）。修复：commit `c3e93e7 chore: strip utf-8 bom from runtime/graph.py` 去 BOM 3 字节并把修改日期 2026-07-15 → 2026-07-20。重跑全量回归 → **5257 passed, 2 skipped, 0 failed**。未跟踪文件保留：`.claude/worktrees/`、`artifacts/`、`docs/audits/2026-07-15-post-july14-game-log-audit.md`。Worktree 收尾：保留 `.worktrees/post-july14-full-repair`（独立未完成工作，分支 `codex/post-july14-full-repair` HEAD=`4ea9390`，未合并）、`.claude/worktrees/{name}`（harness 自管，HEAD=`27cc0ed`）、`C:/Users/10655/.codex/worktrees/72d6/wofkill`（codex 自管 detached HEAD）；清理 `.worktrees/semantic-public-evidence-gate`（已合并到 master，HEAD=`c6256a0`）与 `codex/semantic-public-evidence-gate` 分支。
- **本次新增 (ark-dedup)**: `config/models.yaml` 中两个 Ark profile 与 native_minimax 重复覆盖 MiniMax-M3/M2.7（ark 侧的 `minimax-m3`/`minimax-m2.7` 已被原生侧取代），按用户要求替换为 Kimi-K2.6 和 DeepSeek-V4-Pro。改动：`ark_minimax_m3` 重命名为 `ark_kimi_k26`（model=`Kimi-K2.6`），`ark_minimax_m2_7` 重命名为 `ark_deepseek_v4_pro_secondary`（model=`DeepSeek-V4-Pro`，temp=0.6 / top_p=0.95 与 primary `ark_deepseek_v4_pro` 区分）；7 处 `model_profile:` 引用（ark_deepseek / ark_kimi / minimax_default.default / speech / deception / night_action）同步重命名。Final Ark 端 4 个 model ID：`Kimi-K2.6`, `deepseek-v4-flash`, `deepseek-v4-pro`, `DeepSeek-V4-Pro`。玩家实际分配：p02/p04/p09/p12/judge（minimax_default）→ Kimi-K2.6 via Ark；p07（ark_kimi）→ DeepSeek-V4-Pro via Ark；p05（ark_minimax）→ deepseek-v4-flash 不变；p11（ark_glm）→ deepseek-v4-pro 不变；5 个 native_minimax 玩家走 native 端不变。验证：6 个 model ID live API 实测（`ark.cn-beijing.volces.com/api/coding/v3`）全部 HTTP 200，全部产出 `reasoning_content` + `completion_tokens_details.reasoning_tokens`，`_generate_openai_compatible` 解析路径无回归。更新：`tests/model_gateway/test_minimax_provider_routing.py`（原 `test_ark_minimax_m3_model_profile_exists` → `test_ark_kimi_k26_model_profile_exists`，新增 `test_ark_deepseek_v4_pro_secondary_model_profile_exists` 与 `test_ark_dedup_no_longer_wraps_minimax_models`）。
- **本次新增 (minimax-native-key-routing)**: 历史 soak 暴露 native MiniMax endpoint (`api.minimaxi.com/v1`) 错误回退 Ark 鉴权键并返回 HTTP 401。当前修复：`providers/openai.py` 使用 `_resolve_api_key_for_base_url(base_url, default_key)`，以同一个最终生效 URL 同时驱动鉴权与 HTTP 请求；仅精确匹配 HTTPS `api.minimaxi.com`，按 `MINIMAX_NATIVE_API_KEY` → `MINIMAX_API_KEY` → MiniMax-scoped `ANTHROPIC_API_KEY` 的优先级解析，缺少合法键时在请求前 fail closed，非 MiniMax endpoint 保持默认键。离线验证：`TestNativeMiniMaxApiKeyRouting` 的 9 项请求边界测试覆盖 dedicated/vendor/scoped key 优先级、缺钥阻断、provider 默认 URL、相似域名、非 HTTPS、非 MiniMax-scoped Anthropic 配置与 Ark endpoint；相关三文件回归 61 项通过。真实 probe 因本机 socket `WinError 10013` 与凭证外发策略而在到达供应商前被阻止，因此当前证据仅证明离线 resolver/请求边界，不能宣称当前实现已在线返回 HTTP 200，也不能宣称线上 HTTP 401 已消失。
- **本次新增 (native-minimax-routing)**: 5 个玩家（p01/p03/p06/p08/p10）切到 MiniMax 自有 `https://api.minimaxi.com/v1` OpenAI-compatible endpoint，调用方式按用户给的 `OpenAI()` 客户端示例（`model="MiniMax-M3"` / `"MiniMax-M2.7"`，`extra_body={"reasoning_split": True}`）。固定 3:2 分配（M3:p01/p03/p08，M2.7:p06/p10），近似 2:1 权重。架构改动：C 方案 —— `ModelConfig` 新增 `base_url: str | None` + `extra_body: dict[str, Any]` 字段（`__hash__ = None` 因 dict 不可哈希，路由侧把 ModelConfig 作为值对象不放入 set/dict key，无影响）；`router_selection._resolve_config` + `_resolve_fallback_model` 从 `model_profile.base_url` / `.extra_body` 读；4 个 provider（OpenAI/Anthropic/MiniMax Anthropic/GLM）的 `generate()` 用 `config.base_url or self._base_url` 覆盖 URL，`_generate_openai_compatible` + anthropic/minimax 在 payload 末尾用 `payload.setdefault(k, v)` 合并 extra_body（不覆盖显式字段）。YAML 改动：`config/models.yaml` 加 `minimax_native_m3` / `minimax_native_m2_7`（model_profile + llm_profile 两层），5 玩家重绑；其余 7 个玩家 + 4 个 `ark_*` profile 完全不动，`OPENAI_BASE_URL` 全局 env 不变。Fallback 仍走异厂商 `provider: minimax` + `minimax_m27_default`（v1.1.4 cross-provider rule）。变更文件：`werewolf_agent/model_gateway/usage_records.py`、`router_selection.py`、`providers/{openai,anthropic,minimax,glm}.py`、`config/models.yaml`；新增 `tests/model_gateway/test_per_profile_url_and_extra_body.py`（21 测试：ModelConfig 字段 + `_resolve_config` 字段提取 + base_url 覆盖 + extra_body 合并 + setdefault 不覆盖 + 4 个 provider 各验）；更新 `tests/model_gateway/test_minimax_provider_routing.py`（断言 minimax_default 5 人、native_m3 3 人、native_m2_7 2 人，加 native endpoint + base_url + extra_body + cross-provider fallback 测试）。验证：`tests/model_gateway tests/agents` 1013 passed；`tests/integration tests/runtime`（跳 live）1383 passed；新文件 21 passed；路由测试 8 passed。开放风险：暂无 1a-verify 真实对局跑通 native endpoint（mock 测试仅覆盖 payload 形状，不验 SSL/auth）。
- **本次新增 (srp-rag-splitting)**: Task 11.8 / Task 14 完成。`retriever.py` 保留 `StrategyRetriever`/旧 priority helper/`role_phase_matches` 等兼容入口，排序、过滤、score merge、reranker flow、result shaping 主实现迁入 `retrieval_ranking.py`；`vector_store.py` 保留 factory/protocol/re-export/monkeypatch shim，local TF-IDF 迁入 `local_vector_store.py`，hash embedding、`PgVectorStore`、`SiliconFlowVectorStore` 迁入 `embedding_vector_store.py`。兼容修复覆盖 `_QUALITY_ORDER`/`_CASE_TYPE_PRIORITY` rebind、`StrategyRetriever._score` monkeypatch、`vector_store._tokenize`/`_text_to_embedding` monkeypatch 和 pgvector legacy embedding。提交：`2b22ac4`、`8bc75e1`、`d82aab9`、`84d5779`。review gate：spec 复审通过，quality 复审 Ready to merge。验证：`tests/rag/test_query_processing.py tests/rag/test_vector_store_split.py` 13 passed；`tests/rag/test_rag_hardening.py` 55 passed；`tests/rag/test_rag.py` passed；计划 focused 组合 `test_rag.py test_retrieval_eval.py test_clients.py test_query_processing.py test_vector_store_split.py -n 0` passed；`compileall werewolf_agent\rag` passed；`ruff F401,F841` passed；`git diff --check` passed。早期 `test_retrieval_eval.py` setup 超时风险已用 repo-local basetemp + `-n 0` 复核正常退出。
- **本次新增 (reflection-parser-format)**: `reflection.py` 新增 module-level helper `_iter_section_items(body, *, min_chars=6)` + `_LEADING_ITEM_PREFIX_RE`（接受 `-`/`•`/`*` bullet + `1.`/`1、`/`1)` 数字编号[需尾随空格] + prose 段落；过滤空行/短行/短冒号 preamble `endswith(("：",":")) and len<=8`，B1 修防 `本局做对的:` 6 字符泄漏）。`_extract_llm_mistakes`/`_extract_llm_strengths` 内层 `splitlines` 循环改为 `for line in _iter_section_items(body)`，保留 truth-token drop / scrub / cap 3/2 / `auto_verified=False`/`fact_basis="llm_transferable"`。根因：真实 LLM（g_1600154180）用数字编号+prose，旧 bullet-only gate 全 drop → mistake/strength 0 → 0 approved。`test_reflection_v2.py` +11 测试（helper numeric/中文变体/prose/bullet/preamble-colon + parser numeric/cap/truth-token + strengths numeric/truth-token + 端到端 approved `quality_score>=0.85`），更新现有 `test_extract_llm_mistakes_skips_preamble_non_bullet_lines`（preamble 缩至 ≤8 字符匹配 B1 阈值 + 注释）与 `test_iter_section_items_accepts_numeric_chinese_variants`（变体加尾随空格匹配 regex）。65 passed。`analyze_reflection_score.py` 无需 fixture 改动，prose/numeric 路径均到 approved。
- **本次新增 (deadcode-sweep)**: 全项目 vulture 死代码扫描清理（`--min-confidence 80` 从 10→0）。清：9 unused imports（judge/app/reflection/openai/agent_adapter）+ 4 JudgeXxxInput 死 class（schemas.py，零 caller）+ 23 散点死符号（agents/cognition/memory/rag/eval/runtime/tools 跨 16 文件，含 export_report/_references_not_current_evidence/_uses_*_pipeline/_clean_list_items/build_action_tool_schema/line_for/facts_on_day/black_gold_targets_by_claimant/add_relations/reflections_by_role/add_texts/document_ids/is_beneficial/materialize/apply_gate_result/GateResult/import_usage_records/add_batch/_action_result_to_dict/ExternalHistory/ExternalProfileProvider，+ 连锁孤立 import + 3 死测试）+ player.py 27 个 output_parser/tool_schema 薄 wrapper（被取代，保留 8 live）+ player.py 8 连锁孤儿 import（unescape/ValidationError/ActionTrace 等）+ context.py `build_agent_context` 死参数 `ablation_toggles`。方法：vulture 60/80 扫描 → 5 subagent 分模块 triage（grep 验证 caller，排除 API FastAPI/pydantic/property/dunder/继承/测试误报）→ 确认清单 → 批量删 + compileall + 全量绿。全量 3410 passed, 1 skipped（零回归）。
...
## deadcode-sweep — 2026-06-19 (已完成)

**背景**: deadcode-cleanup 之后的全项目死代码扫描清理。用 vulture（AST 级死代码检测）系统扫描 → grep 验证 caller → 排除误报（API FastAPI 路由/Pydantic 模型/property/dunder/继承/测试/接口完整性）→ 确认真死 → 批量删。

**改动**（5 commit，`codex/deadcode-sweep` 分支）:

| 批次 | 内容 |
|---|---|
| 9 unused imports + 4 JudgeXxxInput class | judge.py/app.py/reflection.py/openai.py/agent_adapter.py 的死 import + schemas.py 零 caller 的 4 个 Judge structured-input class |
| 23 散点死符号 | agents（export_report/_references_not_current_evidence/_uses_*_pipeline/_clean_list_items/build_action_tool_schema）+ cognition/memory（line_for/facts_on_day/black_gold_targets_by_claimant/add_relations/reflections_by_role/add_texts/document_ids）+ rag/eval（is_beneficial/materialize/apply_gate_result/GateResult/import_usage_records/add_batch）+ runtime（_action_result_to_dict）+ tools（ExternalHistory/ExternalProfileProvider）；含连锁孤立 import + 3 死测试 |
| player.py 27 wrapper | output_parser/tool_schema 薄 wrapper（_repair_*/_extract_*/_infer_*/_sanitize_* 等，被取代），保留 8 live wrapper（_parse_action 等） |
| player.py 8 孤儿 import | unescape/ValidationError/ActionTrace/FactionGoal/PrivateIntent/RiskFlag/SeerStance/VoteBasis |
| context.py 死参数 | `build_agent_context` 的 `ablation_toggles`（未用 + 无 caller 传） |

**验证**:
- 方法：vulture 60（601 候选）→ 排除 unused variable 噪音 + API 层 FastAPI 误报 → 5 subagent 分模块 triage（grep 验证 cross-file caller）→ ~45 确认死符号 → 批量删 + compileall + 全量绿。
- vulture `--min-confidence 80`：**10 → 0**（高置信死代码全清）。
- `python -m pytest -q -o addopts=""` → **3410 passed, 1 skipped**（零回归，-3 删掉的死测试）。
- compileall 干净。

**保留（非死代码，误报或未接线基础设施）**:
- API 层（`api/routes/*` 路由函数、`api/schemas.py` Pydantic 请求模型、`create_app`）——FastAPI 装饰器/反射注册，vulture 看不到，活。
- `storage/*` repository 方法——`GameRepository` 协议接口完整性（部分未直接调但实现接口）。
- `memory/reflection_effectiveness.py::evaluate_reflection_effectiveness`——spec 评估能力待接线。
- `cognition/claim_credibility`+`public_evidence` 的 `snapshot`/`from_snapshot`——跨进程 resume 持久化待接线。
- Pydantic `@model_validator`/`@field_validator`/`@property`/dunder/被继承方法——vulture 误报。
- vulture `--min-confidence 60` 剩余 ~250 候选（method/variable）基本是上述误报类别，triage 后确认真死已清。
...
1. skill 注入已有结构化 list，但每项仍主要是 `{skill, advice, confidence}`，语义接近自由文本。
2. role/phase/task_type 匹配后可能注入较多技能建议，缺少最终 top-N relevance gate。
3. prompt builder 只渲染单行 advice，不能表达适用局面、风险和禁止套用边界。

**改动**:

| 项目 | 问题 | 修复 |
|---|---|---|
| skill schema | `SkillOutput` 不能承载结构化战术 frame | 新增 `SkillAdviceFrame`，并在 `SkillOutput` 增加可选 `advice_frame` |
| 兼容转换 | 12 个 handler 仍返回旧 `prompt_injectable` | `_inject_skill_output()` 增加兼容层，将旧输出转换为 frame dict，保留旧 `advice` 字段 |
| relevance gating | 匹配到的技能可能全部注入 | 按 `relevance/confidence` 排序，仅保留 top 3 条 `skill_tactical_advice` |
| prompt 渲染 | skill 建议只是一行 bullet，边界不清晰 | `PlayerPromptBuilder` 渲染“适用局面 / 本轮建议 / 风险 / 不适用信号 / 禁止套用” |

**验证**:

- 已通过 `python -m pytest tests/skills/test_skills.py::TestSkillOutput::test_skill_advice_frame_creation tests/skills/test_skills.py::TestSkillOutput::test_skill_output_can_carry_advice_frame tests/runtime/test_context.py::test_skill_tactical_advice_entries_use_frame_schema tests/runtime/test_context.py::test_skill_tactical_advice_gates_to_top_three_by_relevance -q -o addopts='' -p no:cacheprovider -p no:xdist -p no:xdist.looponfail --basetemp E:\NLP\agent\wofkill\.pytest_tmp`。
- 已通过 `python -m pytest tests/skills tests/runtime/test_context.py tests/agents/test_prompt_builder.py tests/agents/test_player_agent.py -q -o addopts='' -p no:cacheprovider -p no:xdist -p no:xdist.looponfail --basetemp E:\NLP\agent\wofkill\.pytest_tmp` — **439 passed**。
