...
- 2026-07-08 prompt choice 拆分验证记录：新增兼容测试先因缺少 `prompt_choice` 失败；拆分后 `tests/agents/test_prompt_sections.py`、`tests/agents/test_prompt_builder.py` 和 `tests/agents/test_player_agent.py` 通过；production touched-file `ruff F401,F841`、`compileall`、`git diff --check` 均返回 exit code 0；完整 `test_player_agent.py` 文件仍有既有测试文件 F401/F841 债务，本批不作为 Ruff gate；`prompt_output.py` 从 460 行降至 379 行，choice 候选映射、投票/目标候选摘要和 choice prompt 渲染迁移到 `prompt_choice.py`，旧 `PromptOutputMixin` 方法和 `prompt_output` helper 导入保持兼容。
- 2026-07-08 prompt RAG memory 拆分验证记录：新增兼容测试先因缺少 `prompt_rag_memory` 失败；拆分后 `tests/agents/test_prompt_memory.py`、`tests/agents/test_prompt_builder.py` 和 `tests/agents/test_prompt_injection_fixes.py` 通过，production touched-file `ruff F401,F841`、`compileall`、`git diff --check` 均返回 exit code 0；`prompt_memory.py` 从 435 行降至 299 行，RAG 案例过滤、prompt-safe 瘦身、卡片渲染和头尾防套用文案迁移到 `prompt_rag_memory.py`，旧 `PromptMemoryMixin._build_rag_hints`、`_slim_rag_hint_items`、`_render_rag_hint_cards` 和 `prompt_memory` helper 导入保持兼容。
- 2026-07-08 router probe 拆分验证记录：新增兼容测试先因缺少 `router_probe` 失败；拆分后 `tests/model_gateway/test_router.py`、`tests/model_gateway/test_router_split_helpers.py` 和 `tests/agents/test_model_router.py` 通过，production touched-file `ruff F401,F841`、`compileall`、`git diff --check` 均返回 exit code 0；`router.py` 从 458 行降至 408 行，结构化 tool-call 支持探测迁移到 `router_probe.py`，旧 `ModelRouter.probe_tool_call_support` 和 `router.probe_tool_call_support` facade 导入保持兼容。
...
- 2026-07-08 night witch node 拆分验证记录：新增兼容测试先因缺少 `night_witch_node` 失败；拆分后 `tests/runtime/test_graph_lifecycle.py::test_night_witch_node_is_split_from_night_specialists_facade`、`tests/runtime/test_night_specialists.py`、`tests/runtime/test_witch_flow.py`、女巫审计顺序与 dead witch guard 测试通过，production touched-file `ruff F401,F841`、`compileall`、`git diff --check` 均返回 exit code 0；`night_specialists.py` 从 509 行降至 286 行，女巫夜间节点迁移到 `night_witch_node.py`，旧 `night_specialists.night_witch` 兼容入口保持可导入。本批顺手优化女巫私有 payload 构造为 `_witch_role_payload` helper，并保留 `wolf_kill_target_id` payload 字段。
- 2026-07-08 player retry hints 拆分验证记录：新增兼容测试先因缺少 `player_retry_hints` 失败；拆分后 `tests/agents/test_player_agent.py` 通过，production touched-file `ruff F401,F841`、`compileall`、`git diff --check` 均返回 exit code 0；`player_action_flow.py` 从 515 行降至 486 行，空响应和缺失 tool call 的重试提示构造迁移到 `player_retry_hints.py`，旧 helper 兼容入口保持可导入。
- 2026-07-08 player persona context 拆分验证记录：新增兼容测试先因缺少 `player_persona` 失败；拆分后 `tests/agents/test_player_agent.py` 通过，production touched-file `ruff F401,F841`、`compileall`、`git diff --check` 均返回 exit code 0；`player.py` 从 516 行降至 447 行，每回合 persona snapshot 解析与曝光记录迁移到 `player_persona.py`，旧 `_attach_persona_snapshot` / `_record_persona_exposure` 兼容入口保持可用。
...
- 2026-07-08 judge dynamic broadcasts 拆分验证记录：新增兼容测试先因缺少 `judge_dynamic_broadcasts` 失败；拆分后 `tests/agents/test_judge_agent.py` 和 `tests/runtime/test_judge_flow.py` 通过，production touched-file `ruff F401,F841`、`compileall`、`git diff --check` 均返回 exit code 0；`judge.py` 从 498 行降至 204 行，唱票、技能引导、计票结果、放逐结果和警长结果的 LLM 动态播报迁移到 `judge_dynamic_broadcasts.py`，旧 `JudgeAgent` 方法和 `judge.py` helper 导入保持兼容。本批顺手将重复的 judge LLM 生成、零 jitter 和空响应判断收敛为 `_generate_judge_text`。
...
- 2026-07-08 local tool memory 拆分验证记录：新增兼容测试先因缺少 `local_tool_memory` 失败；拆分后 `tests/tools/test_tools.py` 和 `tests/memory/test_reflection.py` 通过，production touched-file `ruff F401,F841`、`compileall`、`git diff --check` 均返回 exit code 0；`local_tools.py` 从 476 行降至 372 行，默认 MemoryStore、认知矩阵查询和 review 写入迁移到 `local_tool_memory.py`，旧 `_get_default_memory_store` / `_query_cognition_matrix` / `_write_review` 私有导入路径保持兼容。本批顺手将 cognition matrix 懒初始化收敛为 `_matrix_for_viewer` helper。
- 2026-07-08 game snapshot audit routes 拆分验证记录：新增兼容测试先因缺少 `game_snapshot_audit` 失败；拆分后 `tests/api/test_game_persistence_helpers.py`、`tests/api/test_routes.py`、`tests/api/test_rag_audit_removed.py`、share-summary/auth 相关 API 测试和 world-model audit view 测试通过，production touched-file `ruff F401,F841`、`compileall`、`git diff --check` 均返回 exit code 0；`game_snapshots.py` 从 423 行降至 350 行，RAG 注入审计和世界模型审计端点迁移到 `game_snapshot_audit.py`，主 snapshot 路由注册函数只保留挂载入口。本批顺手将审计角色校验、事件加载和事件 payload 解析收敛为 helper。
