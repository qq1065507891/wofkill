...
- 后续批次已处理：`werewolf_agent/agents/prompt_builder.py` 已继续拆出输出协议、示例、choice 候选摘要和最终提交约束到 `werewolf_agent/agents/prompt_output.py`，并继续拆出稳定 system prompt 区段到 `werewolf_agent/agents/prompt_system.py`，旧 `PlayerPromptBuilder` 方法名保持兼容；`werewolf_agent/runtime/agent_action_pipeline.py` 已继续拆出女巫、预言家、混血儿选主、警徽处置、猎人开枪等特殊/夜间行动到 `werewolf_agent/runtime/agent_special_actions.py`，并继续拆出日间发言/投票/遗言行动到 `agent_day_actions.py` facade、`agent_day_speech_actions.py` 和 `agent_day_vote_actions.py`，旧 pipeline 和 `agent_adapter` 导入路径保持兼容；`werewolf_agent/evaluation/metric_aggregation.py` 已继续拆出阵营、玩家、角色基础 outcome 指标到 `werewolf_agent/evaluation/metric_outcomes.py`，并拆出质量指标到 `werewolf_agent/evaluation/metric_quality.py`，旧 `MetricsAggregator._compute_*` 私有方法名保持兼容。
...
- 仍超过 500 行的非测试文件：`scripts/run_real_game.py`(579)、`werewolf_agent/agents/judge_hitl.py`(576)、`werewolf_agent/agents/action_schemas.py`(562)、`werewolf_agent/agents/player_action_flow.py`(561)、`werewolf_agent/agents/judge.py`(557)、`werewolf_agent/agents/prompt_builder.py`(548)、`werewolf_agent/runtime/private_memory.py`(544)、`werewolf_agent/runtime/graph.py`(542)、`werewolf_agent/runtime/agent_sheriff_actions.py`(535)、`werewolf_agent/runtime/context.py`(533)、`werewolf_agent/evaluation/balance_audit.py`(520)、`werewolf_agent/agents/player.py`(516)、`werewolf_agent/runtime/nodes/night_specialists.py`(509)。
- 建议顺序：下一步按职责拆分 500 行以上文件，优先处理 `prompt_builder.py` 剩余 dynamic user context、`context.py` 剩余上下文组装责任、`private_memory.py` 持久化/摘要/访问边界这类生产核心大文件，脚本类文件可放在生产核心之后。
