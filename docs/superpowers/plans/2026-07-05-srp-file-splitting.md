...
- 仍超过 500 行的非测试文件：`scripts/run_real_game.py`(579)、`werewolf_agent/agents/judge_hitl.py`(576)、`werewolf_agent/agents/judge.py`(557)。
- 建议顺序：下一步处理 `agents/judge.py`；脚本类文件和 judge/HITL 文件可放在生产核心之后，HITL 可在普通 judge 拆分后处理。`private_memory.py`、`graph.py`、`agent_sheriff_actions.py`、`night_specialists.py`、`player_action_flow.py`、`player.py`、`balance_audit.py` 和 `action_schemas.py` 已低于 500 行。
