...
- 仍超过 500 行的非测试文件：`scripts/run_real_game.py`(579)、`werewolf_agent/agents/judge_hitl.py`(576)、`werewolf_agent/agents/action_schemas.py`(562)、`werewolf_agent/agents/player_action_flow.py`(561)、`werewolf_agent/agents/judge.py`(557)、`werewolf_agent/runtime/graph.py`(542)、`werewolf_agent/runtime/agent_sheriff_actions.py`(535)、`werewolf_agent/evaluation/balance_audit.py`(520)、`werewolf_agent/agents/player.py`(516)、`werewolf_agent/runtime/nodes/night_specialists.py`(509)。
- 建议顺序：下一步按职责拆分 500 行以上文件，优先处理 `player_action_flow.py` provider 调用/retry/fallback 边界、`graph.py` 图构建/节点注册边界这类生产核心大文件；`private_memory.py` 已低于 500 行，剩余持久化/摘要/访问边界可后续按风险继续小批次处理，脚本类文件可放在生产核心之后。
