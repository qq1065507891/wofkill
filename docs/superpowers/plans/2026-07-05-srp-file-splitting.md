...
- 后续批次已处理：`werewolf_agent/agents/prompt_builder.py` 已继续拆出输出协议、示例、choice 候选摘要和最终提交约束到 `werewolf_agent/agents/prompt_output.py`，并继续拆出稳定 system prompt 区段到 `werewolf_agent/agents/prompt_system.py`，旧 `PlayerPromptBuilder` 方法名保持兼容；`werewolf_agent/runtime/agent_action_pipeline.py` 已继续拆出女巫、预言家、混血儿选主、警徽处置、猎人开枪等特殊/夜间行动到 `werewolf_agent/runtime/agent_special_actions.py`，旧 pipeline 和 `agent_adapter` 导入路径保持兼容。
