...
- 后续批次已处理：`werewolf_agent/agents/prompt_builder.py` 已继续拆出输出协议、示例、choice 候选摘要和最终提交约束到 `werewolf_agent/agents/prompt_output.py`，并继续拆出稳定 system prompt 区段到 `werewolf_agent/agents/prompt_system.py`，旧 `PlayerPromptBuilder` 方法名保持兼容；`werewolf_agent/runtime/agent_action_pipeline.py` 已继续拆出女巫、预言家、混血儿选主、警徽处置、猎人开枪等特殊/夜间行动到 `werewolf_agent/runtime/agent_special_actions.py`，旧 pipeline 和 `agent_adapter` 导入路径保持兼容；`werewolf_agent/evaluation/metric_aggregation.py` 已继续拆出阵营、玩家、角色基础 outcome 指标到 `werewolf_agent/evaluation/metric_outcomes.py`，旧 `MetricsAggregator._compute_*` 私有方法名保持兼容。
...
- 仍超过 500 行的非测试文件：`werewolf_agent/runtime/agent_action_pipeline.py`(648)、`werewolf_agent/evaluation/metric_aggregation.py`(593)、`scripts/run_real_game.py`(582)、`werewolf_agent/agents/judge_hitl.py`(576)、`werewolf_agent/agents/player_action_flow.py`(564)、`werewolf_agent/agents/action_schemas.py`(563)、`werewolf_agent/agents/judge.py`(557)、`werewolf_agent/agents/prompt_builder.py`(553)、`werewolf_agent/runtime/context.py`(546)、`werewolf_agent/runtime/private_memory.py`(544)、`werewolf_agent/runtime/graph.py`(542)、`werewolf_agent/runtime/agent_sheriff_actions.py`(539)、`werewolf_agent/evaluation/balance_audit.py`(520)、`werewolf_agent/agents/player.py`(516)、`werewolf_agent/runtime/nodes/night_specialists.py`(510)。
- 建议顺序：下一步按职责拆分 500 行以上文件，优先处理 `agent_action_pipeline.py` 剩余日间发言/投票责任、`metric_aggregation.py` 剩余质量/安全/成本责任、`prompt_builder.py` 剩余 dynamic user context 这类生产核心大文件，脚本类文件可放在生产核心之后。
...
- `werewolf_agent/runtime/agent_action_pipeline.py`，2026-07-08 后续批次已拆分特殊/夜间行动到 `agent_special_actions.py`，旧 pipeline 与 `agent_adapter` 继续作为兼容入口重新导出原函数名；本次同时在新模块内部合并重复的 state 上下文和审计参数透传代码，不改变行动规则或公共 API。
...
- `werewolf_agent/agents/prompt_builder.py`，2026-07-08 后续批次已拆分稳定 system prompt 区段到 `prompt_system.py`，旧 `PlayerPromptBuilder._build_*` 方法名继续通过 mixin 暴露。
...
- `werewolf_agent/evaluation/metric_aggregation.py`，2026-07-08 后续批次已拆分基础 outcome 指标到 `metric_outcomes.py`，旧 `MetricsAggregator` 私有方法名继续作为类属性兼容入口。
