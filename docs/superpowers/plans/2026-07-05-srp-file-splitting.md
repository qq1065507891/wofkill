...
- 仍超过 500 行的非测试文件：`scripts/run_real_game.py`(579)、`werewolf_agent/agents/judge_hitl.py`(576)、`werewolf_agent/agents/action_schemas.py`(562)、`werewolf_agent/agents/judge.py`(557)、`werewolf_agent/runtime/agent_sheriff_actions.py`(535)、`werewolf_agent/evaluation/balance_audit.py`(520)、`werewolf_agent/agents/player.py`(516)、`werewolf_agent/runtime/nodes/night_specialists.py`(509)。
- 建议顺序：下一步处理 `agent_sheriff_actions.py` 或 `night_specialists.py` 这类 runtime 节点责任；随后处理 `agents/player.py` facade 或 `evaluation/balance_audit.py`。`player_action_flow.py`、`private_memory.py` 和 `graph.py` 已低于 500 行，脚本类文件可放在生产核心之后。
