...
| `werewolf_agent/runtime/timers.py` | 运行时 | 计时器 | 中 |
...
- **职责**：LangGraph 状态图编排、节点实现、上下文构建、跨局记忆/反思、策略指令、计时、检查点、HITL、复盘。
- **核心**：
  ...
  - 私有 action 拆分布局（多次 SRP 拆分后）：
    ...
    - `agent_action_pipeline.py` — `_call_agent` 内部 helper facade
...
| # | 启动项 | 命令/函数 | 配置来源 |
| - | - | - | - |
| 1 | 真实 LLM 一局 | `python scripts/run_real_game.py [--seed N --max-steps 500 --timeout 120]` | `.env`（`ANTHROPIC_API_KEY` 或 `GLM_API_KEY`、`ANTHROPIC_BASE_URL`、`WEREWOLF_USE_LLM_AGENTS=1`、`WEREWOLF_MODEL_CONFIG=config/models.yaml`）+ CLI |
...
| 输入 | `--seed 42 --max-steps 500 --timeout 120`、`.env` |
...
| 核心 | (1) `enter_night` phase=night；(2) 4 狼轮流 LLM 发言；(3) 首狼 Pydantic `WolfTeamPlan`（fake_seer/pusher/hooker/deep_cover + 击杀目标主备 + public_story + reasoning；`reasoning` 仅狼队可见；LLM 失败回退到 `_build_fallback_wolf_team_plan`，emit `wolf_team_plan_fallback` 审计）；(4) 4 狼投票达成一致，超时 `no_kill_timeout` |
...
| 核心 | 1) `free_discussion` 按 `gs.speech_order`（警长决定或无警长随机起点 + 座次方向）轮流 `_call_agent(player_id, task_type="SPEECH")`；超时记 `speech_timeout`。2) `summarize_positions`（仅当 `cfg.summarize_after_discussion=True`）LLM 压缩每个人立场。3) `sheriff_endorse`（仅当有警长且 D2+）警长归票。4) `day_vote` 12 玩家 LLM 投票到 `state["exile_votes"]` |
...
| 10 | `wolf_discussion` | 狼讨论 | `_call_agent(wolf_id, "WOLF_DISCUSSION")` × 4 | 4 狼 LLM 发言 | `run_player_action_flow` | `state.wolf_team_plan` 部分填 |
| 11 | `wolf_team_plan_node` | 狼队计划 | `_call_agent(head_wolf, "WOLF_TEAM_PLAN")` | LLM 产 Pydantic | `WolfTeamPlan` schema validate；fallback 路径 emit `wolf_team_plan_fallback` | `state.wolf_team_plan = {...}` |
| 12 | `wolf_consensus` | 狼最终决策 | `_legacy_wolf_consensus` | 决定 kill/no_kill | 受 `wolf_discussion_seconds` 限制 | `state.wolf_kill_target_id = p07` |
| 13 | `night_witch` | 女巫用药 | `_call_agent(witch, "WITCH_NIGHT")` | LLM 选解药/毒药 | `_select_output_mode("target_choice")` + `engine.legal_witch_actions` 校验 | `state.use_antidote / poison_target_id` |
| 14 | `night_seer` | 预言家验人 | `_call_agent(seer, "SEER_NIGHT")` | LLM 选目标 | `engine.check_alignment` | `state.seer_action_trace` |
| 15 | `first_night_hybrid_master` | 混血儿选主 | `_call_agent(hybrid, "HYBRID_MASTER")` | LLM 选 master | `engine.choose_master` | `state.hybrid_master_target_id` |
...
| 19 | `night_death_last_words` | 遗言 | `_call_agent(dead_id, "LAST_WORDS")` | LLM 写遗言 | `run_player_action_flow` | `events` += [last_words] |
| 20 | `sheriff_first_day_entry` (D1) | 警长入口 | `route_after_sheriff_*` | 警长流程 | 串行 registration/speech/withdraw/vote | `gs.sheriff_id` |
| 21 | `free_discussion` | 自由讨论 | `_call_agent(p, "SPEECH")` × N | LLM 发言 | `run_player_action_flow` | `events` += [speech] |
...
| 24 | `day_vote` | 投票 | `_call_agent(p, "VOTE")` × N | LLM 投票 | `output_mode=target_choice` | `state.exile_votes` |
...
| 27 | `exile_last_words` | 放逐遗言 | `_call_agent(t, "LAST_WORDS")` | LLM 写遗言 | | |
| 28 | `resolve_hunter_shot` | 猎人开枪 | `_call_agent(hunter, "HUNTER_SHOT")` | LLM 选目标 | `engine.apply_death` | `gs.deaths` += [hunter_shot] |
...
flowchart LR
    ...
    J[Graph.stream] --> K[per node: _call_agent(player_id, task_type)]
...
| `speech_timed_out` | `bool` | `False` | | | 发言超时 |
| `speech_seconds_limit` | `int` | 0 | | | 发言时限 |
...
| `agent_registry` | `Any` | `None` | _build_runtime_state | _call_agent | Agent 表 |
...
| `runtime_timer` | `Any\|None` | `None` | _build_runtime_state | | 计时器 |
| `consecutive_no_exile_days` | `int` | 0 | | | 连续无放逐天数 |
| `agent_call_timeout` | `float` | 0 | | | LLM 调用超时 |
...
- **私有状态**：
  - 每个 player 的 `state["wolf_team_plan"]` / `state["seer_action_trace"]` / `state["witch_action_trace"]` / `state["hybrid_master_target_id"]` / `state["use_antidote"]` / `state["poison_target_id"]` / `state["wolf_kill_target_id"]` — 仅在 LLM 调用 `_call_agent(player_id, ...)` 时通过 `build_agent_context` 选择性注入 `context.visible_world_state`（`VisibilityPolicy` 硬过滤）。
...
- `wolf_discussion`：4 狼按 `p01..p12` 中 4 狼的固定顺序轮流 LLM 发言（每人 `task_type=WOLF_DISCUSSION`），用 `_build_wolf_kill_directive` 输出"我倾向刀谁"，用 `_single_wolf_vote` 票选；狼队 timer 限 `wolf_discussion_seconds`。
...
- `wolf_consensus`：4 狼多轮投票达成一致，超时默认 `no_kill_timeout`；最终 `state.wolf_kill_target_id` 写入。
...
- **多狼意见不一致**：`wolf_consensus` 多轮投票，直到全部一致或 timer 到。
...
- 任何 `_call_agent(p, ...)` 走完整 `run_player_action_flow` → `ModelRouter.generate` → `parse_action` → `ActionValidator` → 落 `GameEvent`；所以 LLM 输出**只能通过 GameEvent 间接影响 GameState**，不能直接改 `gs.players[p].alive` 等。
...
WEREWOLF AGENT - REAL LLM GAME
  ...
  Test model:  provider=anthropic model=claude-opus-4-7 timeout=180s
  Calling API...
...
- **`_call_agent` 的私有 trace 与跨局 cognition_state_manager 联动**：源码已读各自模块，但实际 reducer 行为（哪些 trace 进 cognition_state）未深追。
